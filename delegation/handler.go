...
func HandleMsgDelegatorBond(ctx sdk.Context, msg MsgDelegatorBond, k Keeper, contractCaller helper.IContractCaller) sdk.Result {
	...
	k.BondDelegator(ctx, msg.ID, types.ValidatorID(eventLog.ValidatorId.Uint64()), eventLog.Amount, lastUpdated)

	resTags := sdk.NewTags(
		tags.DelegatorID, []byte(strconv.FormatUint(delegator.ID.Uint64(), 10)),
		tags.ValidatorID, []byte(strconv.FormatUint(delegator.ValID.Uint64(), 10)),
	)
...
func HandleMsgDelegatorUnBond(ctx sdk.Context, msg MsgDelegatorUnBond, k Keeper, contractCaller helper.IContractCaller) sdk.Result {
	...
	k.UnBondDelegator(ctx, msg.ID, lastUpdated)
