...
func HandleMsgDelegatorJoin(ctx sdk.Context, msg MsgDelegatorJoin, k Keeper, contractCaller helper.IContractCaller) sdk.Result {
	...
	delegator, err := contractCaller.GetDelegatorInfo(msg.ID)
	if err != nil {
		k.Logger(ctx).Error(
	...
	if _, ok := k.GetDelegatorInfo(ctx, msg.ID); ok {
	...
	newDelegator := hmTypes.Delegator{
		ID:          validator.ID,
	...
	resTags := sdk.NewTags(
		tags.delegatorID, []byte(strconv.FormatUint(newDelegator.ID.Uint64(), 10)),
	)
